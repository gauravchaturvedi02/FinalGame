...
class maryo:
    ...
    def __init__(self):
        ...
        self.imagerect = self.image.get_rect()
        self.imagerect.topleft = (60,window_height/2)
        self.score = 0
    ...
    def update(self):
        ...
        if (moveleft and (self.imagerect.left > 0)):
            self.imagerect.centerx -=self.speed
            self.score += 1
    ...
    def return_heightM(self):

        h = self.imagerect.top
        return h

    def return_pos(self):
        pos= self.imagerect.centerx
        return pos
...
class fireballs:
    ...
    def __init__(self):
        self.image = load_image('fireball.png')
        self.imagerect =self.image.get_rect()
        self.height = player.return_heightM()+10
        self.surface = pygame.transform.scale(self.image, (20,20))
        self.imagerect = pygame.Rect(player.return_pos()+5, self.height, 20, 20)

    def update(self):
            self.imagerect.right += self.firespeed
...
def firehitsdragon(dragonrect,fire_list):        #to check when fire hits dragon
    for F in fire_list:
        if dragonrect.colliderect(F.imagerect):
            return True
        return False
    

def mariohitsdragon(playerrect,dragonrect):    #to check if mario has hit the Dragon 
...
while True:
    ...
    while True:     #the main game loop
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_KP_ENTER:
                    newfire = fireballs()
                    fire_list.append(newfire)
                    for F in fire_list:
                        fireballs.update(F)

                    for F in fire_list:
                        if F.imagerect.right >= window_width:
                            fire_list.remove(F)
        ...
        if mariohitsdragon(player.imagerect,Dragon.imagerect):
            if player.score > topscore:
                topscore = player.score
            break
        
        if firehitsdragon(Dragon.imagerect, fire_list):
            player.score +=10
